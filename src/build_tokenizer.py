...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Checkpoint for the model. (default: 'openai-community/gpt2')",
    )
    parser.add_argument(
        "--data_ckpt",
        type=str,
...
def main(args: argparse.Namespace) -> None:
    # loading the dataset
    dataset = load_dataset("csv", data_files=args.processed_data_file_path)
    print(f"Loaded dataset\n{dataset}")
    print(f"Sample record\n{dataset['train'][0]}")
    # loading the base model tokenizer
    base_tokenizer = AutoTokenizer.from_pretrained(args.model_ckpt, model_max_length=args.model_max_length)
    ...
    dataset_iterator = get_dataset_iterator(dataset["train"], batch_size=args.iterator_batch_size)
    tokenizer = base_tokenizer.train_new_from_iterator(dataset_iterator, vocab_size=args.vocab_size)
